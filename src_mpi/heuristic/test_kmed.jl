...
function init_population_kmeans(data, k)
    ...
    dmat = pairwise(SqEuclidean(), data, dims=2)
    for i in 1:iters
        Random.seed!(seeds[i])
        # kmedoids
        clst_rlt = kmedoids(dmat, k)
        kmed_centers_coors[i, :, :] = data[:, clst_rlt.medoids]
        kmed_UBs[i] = clst_rlt.totalcost
        kmed_centers_indexs[i, :] = clst_rlt.medoids
    end

    return kmed_UBs, kmed_centers_coors, kmed_centers_indexs
end
...
@timeit to "kmeans" begin
    kmed_UBs, kmed_centers_coors, kmed_centers_indexs = init_population_kmeans(X, k)    
